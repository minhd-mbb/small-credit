...
type BaseLoanRateManagementProps = {
  initialRate: string;
  initialUpdatedAt: string | null;
  synchronizedBanks: number;
};

export function BaseLoanRateManagement({
  initialRate,
  initialUpdatedAt,
  synchronizedBanks,
}: BaseLoanRateManagementProps) {
  ...
  async function saveBaseRate() {
    ...
    if (!Number.isFinite(rate) || rate < 0 || rate >= 50) {
      setError("Lãi suất vay cơ bản phải là số hợp lệ và nhỏ hơn 50%.");
      return;
    }

    setIsSaving(true);
    const response = await fetch("/api/loans/base-rate", {
      method: "PATCH",
    ...
    if (!response.ok) {
      setError(payload.error ?? "Không thể cập nhật lãi suất vay cơ bản.");
      return;
    }
    ...
    setMessage("Đã cập nhật lãi suất vay cơ bản và đồng bộ tới các bank.");
  }
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          <p className="text-xs font-bold uppercase text-[var(--text-muted)]">
            Lãi suất vay cơ bản hệ thống
          </p>
          <h2 className="font-display mt-2 text-xl font-bold text-[var(--text-primary)]">
            Áp dụng làm chuẩn cho toàn bộ bank
          </h2>
          <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-[var(--text-secondary)]">
            Khi Admin cập nhật mức này, lãi suất vay cơ bản của từng bank và
            lãi suất vay theo kỳ hạn được tự động điều chỉnh theo tỷ lệ hiện tại.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-[var(--border-card)] bg-[var(--primary-light)] p-4">
          ...
          >
            <Save size={17} />
            {isSaving ? "Đang lưu" : "Lưu lãi suất vay"}
          </Button>
        </div>
      </div>
